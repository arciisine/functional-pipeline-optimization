...
\begin{enumerate}
  ...
  \item \textbf{Evaluate} At runtime, we will verify that the input is indeed a list, and also verify that the set of \pipelineoperators are valid for conversion to a \inlinecode{for} loop.  
  \item \textbf{Compile} Once we have a valid \pipeline, we will then generate the resultant \inlinecode{for} loop.  We take into consideration closed variables to handle re-assigning modified variables on return.   
