...
The transformation process only deals with a single operator at a time.  Given something as simple as \inlinecode{filter(x => x > 2)}, this could be translated into \inlinecode{if (x <= 2) continue} assuming the code is running a \inlinecode{for} loop.
...
\begin{enumerate}
  ...
  \item \textbf{Rewrite Local Variables and Parameters}
    Once the AST is provided, we rewrite all function parameters and references to \inlinecode{this} to allow for multiple operations to reside within the same \inlinecode{for} loop.  This means that any variables of the same name, from separate operators, would collide and produce incorrect code.  
\end{enumerate}