...
The end result will be measured as \textbf{Processing Time}. This is the number of nanoseconds it takes to process one unit of input.  Given a scenario, input size and iteration count, we will see how the different algorithms perform.   The times will be determined using not the wall clock values, but a high resolution timer provided by the Node.js engine \cite{nodehr16}.  This is fairly accurate but is still affected by garbage-collection, and so there are outliers that do show up. To handle some of these outliers we focus on the median, as well as the weighted of average of the central 80\% of the data (dropping the top and bottom 10\%).
